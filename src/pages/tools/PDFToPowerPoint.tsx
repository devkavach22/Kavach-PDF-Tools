  ...
  return (
    <div className="relative flex flex-col min-h-screen bg-slate-50 font-sans text-slate-900 overflow-x-hidden">
       <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <motion.div animate={{ opacity: [0.4, 0.6, 0.4] }} transition={{ duration: 20, repeat: Infinity }} className="absolute -top-[20%] left-[10%] w-[60vw] h-[60vw] bg-orange-200/40 rounded-full blur-[120px]" />
      </div>
          ...
          <div className="max-w-7xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center space-y-3">
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-orange-100 border border-orange-200 animate-float">
                <LucidePresentation className="h-8 w-8 text-orange-500" />
              </div>
              <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-red-500 to-amber-500">PDF to PowerPoint</h1>
              <p className="text-lg text-slate-500">Convert your PDF to an editable PowerPoint presentation (.pptx)</p>
            </div>

             <Card className="bg-white/80 backdrop-blur-md shadow-xl border border-slate-200 max-w-3xl mx-auto mt-8">
                <CardHeader>
                ...
                <CardContent className="space-y-6">
                  ...
                  {file && <div className="p-4 rounded-xl border bg-white border-orange-200 shadow-sm"><p className="font-medium text-slate-900">Selected: {file.name}</p></div>}
                  <Button onClick={handleConvert} disabled={!file} className="w-full bg-orange-600 hover:bg-orange-700 text-lg py-6 rounded-xl font-semibold text-white">
